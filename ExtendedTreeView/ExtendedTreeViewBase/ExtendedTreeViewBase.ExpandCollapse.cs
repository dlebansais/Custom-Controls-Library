...
public abstract partial class ExtendedTreeViewBase : MultiSelector
{
    ...
    protected virtual int CountPreviousChildrenExpanded(object item, int index, int excludedIndex)
    {
        ...
        if ((ItemContainer is not null && ItemContainer.IsExpanded) || (ItemContainer is null && IsItemExpandedAtStart) || (ItemContainer is null && IsContent(item) && IsRootAlwaysExpanded))
    ...
    {
        _ = ExpandedChildren.Remove(item);
